...
public class ReferenceModel extends AbstractComponent {
	...
	/* {"type":"string","x-source":"inject","x-form-group":"content","x-form-label":"Content Name Reference","x-form-type":"text"} */
	@Inject
	...
	private String generateReferenceJson() {
		...
		Resource referencedResource = resourceResolver.getResource(
			reference + "/jcr:content"
		);
		if (referencedResource == null) {
			LOG.error(
				"Reference '{}' does not resolve to a resource.",
				reference
			);
			return null;
		}
		try {
			Map referenceMap = modelFactory.exportModelForResource(
				referencedResource,
				PerConstants.JACKSON,
				Map.class,
