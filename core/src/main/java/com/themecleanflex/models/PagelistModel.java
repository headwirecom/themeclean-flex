      ...
      if (page != null) {
        for (PerPage child : page.listChildren()) {
          if (
            ...
            childPages.add(
              new Page(child, levels, getReferences(), excludeSitemap)
            );
          }
        }
      }
  ...
  class Page {
    ...
    public List<ReferencedContent> getReferences() {
      ...
      for (IComponent ref : references) {
        ...
        String contentField = vm.get("contentfield", String.class) != null
      ...
      if (page != null) {
        for (PerPage child : page.listChildren()) {
          ...
          if (levels > 0 && !child.getPath().equals(page.getPath())) {
            childPages.add(
              new Page(child, levels - 1, references, excludeSitemap)
            );
          }
        }
      }
