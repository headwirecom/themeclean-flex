<template>
	<themecleanflex-components-block v-if="isReady" v-bind:model="model">
		<div class="w-full">
			<div
				class="text-black p-2 rounded-r mt-4 border-l-4 shadow-md note-important"
			...
			<form
				class="w-full flex flex-col md:p-4 sm:p-2"
				v-bind:class="{
			...
			>
				<json-forms
					v-bind:class="`w-full mb-4 md:flex md:flex-wrap md:justify-between`"
...
<script>
	const renderers = [...JSONFormsVue2Vanilla.vanillaRenderers];
	...
	export default {
		...
		computed: {
			...
			jsonFormsKey() {
				const { sha256 } = window;
				const str =
					JSON.stringify(this.schema) + JSON.stringify(this.uischema);
		...
		methods: {
			...
			onSubmit(e) {
				if (
					this.model.submitfunction !== "onSubmit" &&
					this.model.submitfunction !== ""
				) {
					...
					console.log(
						"window." + this.model.submitfunction + " not found"
					);
					this.$set(this, "failureText", this.model.failmessage);
					return;
				}
				axios
					.post(this.model.endpointurl, {
						form: this.form,
					})
					.then(() => {
			...
			updateSchema() {
				if (this.model.schema) {
					axios
						.get(this.$helper.pathToUrl(this.model.schema))
			...
			updateUischema() {
				if (this.model.uischema) {
					axios
						.get(this.$helper.pathToUrl(this.model.uischema))
		...
		provide() {
			...
			const customStyles = {
				...
				control: {
					...
					error:
						"flex items-center font-medium tracking-wide text-xs mt-1 ml-1",
				},
				arrayList: {
					addButton: btnStyles,
