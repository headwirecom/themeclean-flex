<template>
  <component
    class="w-full px-5 flex flex-col justify-center"
    ref="section"
    v-bind:class="[classes, colors, model.component, model.extraclasses]"
  ...
  >
    <div
      class="embed-responsive embed-responsive-16by9 w-full h-full top-0 left-0"
...
export default {
  ...
  computed: {
    ...
    styles: function () {
      ...
      if (this.model.toppadding)
        styles.paddingTop = `${this.model.toppadding}px`;
      if (this.model.bottompadding)
  ...
  methods: {
    backgroundStyles() {
      ...
      switch (this.model.backgroundtype) {
        case "image":
          const overlay =
            this.model.overlay === "true" ? `${this.overlayStyle()},` : "";
