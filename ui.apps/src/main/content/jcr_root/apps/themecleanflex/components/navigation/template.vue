<template>
  <themecleanflex-components-block v-bind:model="model">
    ...
    <nav
      class="flex md:flex-row flex-col z-10 w-full"
      v-bind:class="{
    ...
    >
      ...
      >
        <a
          class="p-3 no-underline"
          v-bind:data-per-inline="`model.childrenPages.${i}.title`"
        ...
        >
          <div class="flex flex-col dropdown-list">
            <a
              class="p-3 no-underline"
              v-bind:data-per-inline="`model.childrenPages.${i}.title`"
              v-bind:href="subchild.path + '.html'"
              v-bind:class="
                model.colorscheme === 'dark'
                  ? 'text-gray-200 hover:bg-gray-200 hover:text-black'
                  : 'text-black hover:bg-black hover:text-gray-200'
              "
              v-for="(subchild, i) in child.childrenPages"
              :key="subchild.path || i"
...
export default {
  ...
  computed: {
    isEditAndEmpty() {
      ...
      if (this.model.childrenPages && this.model.childrenPages.length == 0)
...
.flex.dropdown-container:hover .flex.dropdown-list,
.flex.dropdown-container:focus-within .flex.dropdown-list {
  display: flex;
}
</style>
