<template>
  <themecleanflex-components-block v-bind:model="model">
    ...
    <div class="w-full" v-else>
      ...
      v-html="model.title" data-per-inline="model.title"></h2>
      <div class="flex flex-col -mx-3 flex-grow" v-bind:class="{
        ...
        v-bind:class="{
          ...
          <div v-for="(item, i) in model.accordiontoggle" :key="item.path || i"
          v-bind:id="`accordion${_uid}${parseInt(i)+1}`" v-bind:class="{
            ...
            v-on:click="toggleItem(i)">
              <h3 class="text-lg m-0" v-html="item.title" data-per-inline="item.title"></h3>
            ...
            v-bind:style="`height:${active[i] ? heights[i] + 'px' : '0px'};`">
              <div class="p-3" v-html="item.text" data-per-inline="item.text" v-bind:ref="`cardContent${i}`"></div>
            </div>
          </div>
        </div>
      </div>
