<template>
	<themecleanflex-components-block v-bind:model="model">
		...
		<div class="w-full" v-else>
			<h2
				class="text-xl text-center pb-4"
				v-if="model.showtitle == 'true'"
				data-per-inline="model.title"
			>
				{{ model.title }}
			</h2>
			<div
				class="flex -mx-3 flex-grow"
				v-bind:class="{
			...
			>
				<div
					class="img-wrapper px-0 md:px-3"
					v-if="model.showmedia === 'true'"
					v-bind:style="{ flex: `0 0 ${model.mediawidth}%` }"
					v-bind:class="{
						'img-wrapper rounded-none':
							model.roundedcorners == 'none',
						'img-wrapper rounded-sm':
							model.roundedcorners == 'small',
						'img-wrapper rounded': model.roundedcorners == 'medium',
						'img-wrapper rounded-lg':
							model.roundedcorners == 'large',
						'img-wrapper rounded-full':
							model.roundedcorners == 'full',
					}"
				>
					<themecleanflex-components-media
						:model="model"
					></themecleanflex-components-media>
				</div>
				<div
					class="accordion-container px-0 sm:mx-3 flex-grow overflow-hidden"
					v-bind:class="{
            ...
            'rounded-full': model.roundedcorners == 'full'
				...
				>
					<div
						class="accordion-item border-b transition-colors duration-200 ease-in-out"
            ...
            'bg-secondary': active[i]
					...
					>
						<h3 class="accordion-item-header">
							...
							>
								<span
									v-bind:data-per-inline="`model.accordiontoggle.${i}.title`"
									>{{ item.title }}</span
								>
								<svg
									width="16"
						...
						<div
							class="card-content overflow-hidden transition-height"
							role="tabpanel"
							v-bind:style="`height:${
...
<script>
	export default {
		props: ["model"],
		data: function () {
			const numElements = this.model.accordiontoggle
				? this.model.accordiontoggle.length
				: 0;
			return {
			...
			setHeights: function () {
				this.heights.forEach((item, i) => {
					Vue.set(
						this.heights,
						i,
						this.$refs[`cardContent${i}`][0].clientHeight
					);
				});
			},
			toggleItem(i) {
