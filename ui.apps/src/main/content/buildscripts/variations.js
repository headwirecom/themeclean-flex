...
function objectify(array) {
	const obj = array.map((optionObjectsArr) => {
		let combinedName = optionObjectsArr.reduce(
			(combinedName, optionObject) => {
				return combinedName
					? combinedName + "; " + optionObject.name
					: optionObject.name;
			},
			""
		);
		...
		if (entry.isDirectory()) {
			...
			options.forEach((optionName) => {
				if (optionName.endsWith(".json")) {
					const optionJson = fs.readJSONSync(
						root + name + "/" + optionName
					);
					let optionLabel = optionName.substring(
						0,
						optionName.length - 5
					);
