...
function outSample(name, sample, variation = null) {
	...
	const attrs = [
		...
		["sling:resourceType", "themecleanflex/components/" + name],
	];
	const children = [];
...
function buildPage(target, name, samples, readme, variations) {
	...
	if (readme) {
		let md = fs.readFileSync(
			"../fragments/" + name + "/readme.md",
			"utf-8"
		);
		readmeContent = clfrags.intro(md);
	}

	const samplesContent = samples.reduce(
		(val, sample) => val + outSample(name, sample),
		readmeContent
	);
...
function buildVariationPages(target, name, samples, variations) {
	...
	samples.forEach((sample) => {
		...
		out.write(clfrags.home());
		out.write(clfrags.pager());
		const samplesContent = variations.reduce(
			(val, variation) => val + outSample(name, sample, variation)
		);
		out.write(clfrags.container("main", samplesContent));
		out.write(clfrags.pager());
		out.write(clfrags.footer());
...
function forEachComponent(
	target = "src/main/content/jcr_root/content/themecleanflex/pages/library",
	root = "../fragments/"
) {
	const pages = [];
