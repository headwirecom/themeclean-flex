...
function generateReport(out, name) {
	...
	out.forEach((entry) => {
		data.push(
			`<tr><td colspan="3"><h2>${entry.version} ${entry.url}</h2></td></tr>`
		);
		data.push(`<tr>`);
		data.push(
			`<td style="width: 30%" valign="top"><img style="max-width: 100%" src="${entry.name}"></td>`
		);
		data.push(
			`<td style="width: 30%" valign="top"><img style="max-width: 100%" src="../../approved/${entry.name}"></td>`
		);
		data.push(
			`<td style="width: 30%" valign="top"><img style="max-width: 100%" src="diff/${entry.name}"></td>`
		);
		data.push(`</tr>`);
	});
	data.push(`</table`);
...
async function screenShotAt(out, url, page, width, version, name) {
	...
	await page.screenshot({
		path: `${out_path}/${name}-${version}.png`,
		fullPage: true,
	});
