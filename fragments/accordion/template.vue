          ...
          v-bind:class="{
            ...
            <h3 class="accordion-item-header">
              ...
              v-on:click="toggleItem(i)" v-bind:aria-expanded="active[i] ? 'true' : 'false'">
                <span v-bind:data-per-inline="`model.accordiontoggle.${i}.title`">{{item.title}}</span>
            ...
            v-bind:style="`height:${active[i] ? heights[i] + 'px' : '0px'};`">
              <div class="p-3" v-html="item.text" v-bind:data-per-inline="`model.accordiontoggle.${i}.text`"
