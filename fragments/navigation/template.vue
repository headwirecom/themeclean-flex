<template>
  <themecleanflex-components-block v-bind:model="model">
    ...
    v-bind:class="{
      ...
      :key="item.path || i">
        <a class="p-3 no-underline" data-per-inline="child.title" v-bind:href="child.childrenPages.length &gt; 0 ? false : child.path"
        ...
        <div class="self-stretch" v-if="child.hasChildren &amp;&amp; child.childrenPages &amp;&amp; child.childrenPages.length &gt; 0">
          <div class="flex flex-col dropdown-list" v-bind:class="model.colorscheme === 'dark' ? 'theme-dark': 'theme-light'">
            <a class="p-3 no-underline" data-per-inline="subchild.title" v-bind:href="subchild.path"
            ...
            v-for="(subchild, i) in child.childrenPages" :key="item.path || i">{{subchild.title}}</a>
          </div>
        </div>
      </div>
    </nav>
