<template>
	<themecleanflex-components-block v-bind:model="model">
		...
		<div class="flex flex-wrap md:-mx-3" v-else>
			<div
				class="card my-3 px-0 md:px-3"
				v-bind:class="{
            ...
            'rounded-lg': model.roundedcorners == 'large'
				...
				>
					<div
						class="self-center"
						v-bind:style="{ width: item.imagewidth + '%' }"
					>
						<v-lazy-image
							v-bind:class="
								model.showcard === 'true' ||
								model.cardborder === 'true'
									? 'card-img w-full'
									: 'card-img mb-3 w-full'
							"
							v-bind:src="$helper.pathToUrl(item.image)"
							v-bind:alt="item.imagealttext"
							v-bind:src-placeholder="placeholderSrc(item)"
							v-if="item.image"
						></v-lazy-image>
					</div>
					<div
						class="flex flex-grow flex-col justify-between p-0"
						v-bind:class="{
							'p-6':
								model.showcard === 'true' ||
								model.cardborder === 'true',
					...
					>
						<div>
							...
							<div
								...
								v-html="item.text"
								v-bind:data-per-inline="`model.cards.${i}.text`"
							></div>
						</div>
						<a
							class="btn mt-3"
            ...
            'my-3': model.cardborder !== 'true' &amp;&amp; model.showcard !== 'true'
							...
							v-bind:data-per-inline="`model.cards.${i}.buttontext`"
							>{{ item.buttontext }}</a
						>
					</div>
				</div>
...
<script>
	export default {
		...
		methods: {
			showbutton(card) {
				return (
					this.model.showbutton === "true" && card.buttontext.length
				);
			},
			placeholderSrc(item) {
				const size = item.imageSize;
				return size
					? `data:image/svg+xml,%3Csvg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 ${size.width} ${size.height}"%3E%3C/svg%3E`
					: null;
			},
		},
	};
