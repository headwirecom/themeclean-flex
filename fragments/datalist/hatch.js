module.exports = {
	convert: function ($, f) {
		...
		f.addIf(
			mobileSelectAction,
			'(model.mobiletablestyle === "" || model.mobiletablestyle === "default") && isMobile'
		);
		f.bindEvent(mobileSelectAction, "click", "toggleAllRows");
		f.addIf(
			mobileSelectAction.find(".unchecked"),
			"(!active.every(element => element === true) || active.length === 0)"
		);
		...
		const actionDelete = selectedContainer
			.find(".selected-actions svg")
			.eq(0);
		...
		f.addStyle(
			desktopContainer,
			"height",
			"(model.scrollabletable === 'true') ? model.tableheight + 'px' : 'auto'",
			false
		);
		...
		f.addIf(
			mobileDefaultBody,
			'model.mobiletablestyle === "" || model.mobiletablestyle === "default"'
		);
		...
		f.addStyle(
			mobileDefaultTr,
			"background",
			"active[j] ? 'var(--color-red-500) !important' : ''"
		);

		const mobileDefaultAction = mobileDefaultBody
			.find("td.action-head")
			.eq(0);
		f.addStyle(
			mobileDefaultAction,
		...
		f.addStyle(
			mobileDefaultAction.find(".checked"),
			"fill",
			"active[j] ? 'var(--text-secondary-color) !important' : ''"
		);
		f.bindEvent(
			mobileDefaultAction.find(".action").eq(0),
			"click",
			"toggleRow(j)"
		);

		const mobileDefaultActionSpacer = mobileDefaultBody
			.find("td.mobile-action-spacer")
			.eq(0);
		f.addIf(mobileDefaultActionSpacer, "i !== 0");
		f.addStyle(
			mobileDefaultActionSpacer,
			"background",
			"active[j] ? 'var(--color-red-500) !important' : ''"
		);

		f.mapField(
			mobileDefaultBody.find("td.mobile-header").eq(0),
			"col.header",
			false
		);
		mobileDefaultBody
		...
		f.mapField(
			mobileDefaultBody.find("td.mobile-item").eq(0),
			"data[col.value]",
			false
		);
		f.addStyle(
		...
		const mobileScrollActionTr = scrollMobileBody
			.find("tr.action-row")
			.eq(0);
		const mobileScrollAction = scrollMobileBody
			.find("td.action-item")
			.eq(0);
		...
		f.addStyle(
			mobileScrollAction,
			"background",
			"active[i] ? 'var(--color-red-500) !important' : ''"
		);
		f.addStyle(
			mobileScrollAction.find(".checked"),
			"fill",
			"active[i] ? 'var(--text-secondary-color) !important' : ''"
		);
		f.bindEvent(
			mobileScrollAction.find(".action").eq(0),
			"click",
			"toggleRow(i)"
		);

		const mobileScrollActionAll = mobileScrollActionTr
			.find("td.action-item-all")
			.eq(0);
		f.addIf(
			mobileScrollActionAll.find(".unchecked"),
			"(!active.every(element => element === true) || active.length === 0)"
		);
		f.addElse(mobileScrollActionAll.find(".checked"));
		f.bindEvent(
			mobileScrollActionAll.find(".action").eq(0),
			"click",
			"toggleAllRows"
		);
		...
		mobileScrollBodyTr.prepend(
			"<td class='mobile-header'>{{col.header}}</td>"
		);
