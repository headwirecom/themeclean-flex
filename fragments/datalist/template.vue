<template>
	<themecleanflex-components-block v-bind:model="model">
		...
		<div class="w-full" v-else>
			<div
				class="selected w-full flex justify-between p-3"
				v-bind:class="{
            'hidden': active.filter(element =&gt; element === true).length === 0,
        }"
			>
				<span
					class="action relative cursor-pointer"
					v-if='(model.mobiletablestyle === "" || model.mobiletablestyle === "default") &amp;&amp; isMobile'
				...
				>
					<input
						...
						value
						class="h-100 m-0 opacity-0 p-0 z-10 w-24 absolute"
					/>
					<svg
						class="action-active-svg w-24"
						focusable="false"
						viewBox="0 0 24 24"
						aria-hidden="true"
					>
				...
				<div class="selected-actions">
					<svg
						class="w-24 cursor-pointer action-btn"
						focusable="false"
						viewBox="0 0 24 24"
						aria-hidden="true"
			...
			<div
				...
				v-bind:style="`height:${
					model.scrollabletable === 'true'
						? model.tableheight + 'px'
						: 'auto'
				};`"
			>
				<table
				...
				>
					<thead>
						<tr>
							<th
								class="action-head w-24"
								v-if="model.selectable === 'true'"
								v-bind:class="{
							...
							>
								...
								>
									<input
										...
										data-indeterminate="false"
										value
										class="h-100 m-0 opacity-0 p-0 z-10 w-48 absolute"
									/>
									<svg
										class="action-active-svg w-24"
										focusable="false"
            ...
            'text-right':  col &amp;&amp; col.textalignment === 'right',
					...
					<tbody>
						<tr
							v-for="(data, j) in storageData"
							:key="data.path || j"
							v-if="rowHasData(data, model.columns)"
						>
							<td
								class="action-item w-24"
								v-bind:style="`background:${
									active[j]
										? 'var(--color-red-500) !important'
										: ''
								};`"
								v-if="model.selectable === 'true'"
								v-bind:class="{
								...
								>
									...
									>
										...
										<path
											...
											v-bind:style="`fill:${
												active[j]
													? 'var(--text-secondary-color) !important'
													: ''
											};`"
										/>
									</svg>
            ...
            'align-bottom':  model.rowalignment === 'bottom'
					...
					<caption
						...
						v-bind:style="`caption-side:${model.captionplacement};`"
						data-per-inline="model.captiontext"
					>
						{{
							model.captiontext
						}}
			...
			<div
				class="w-full overflow-y-hidden mobile-table overflow-x-scroll"
				v-bind:class="{
					hidden: !isMobile,
				}"
			>
				...
				>
					<tbody
						...
						v-if="
							...
							model.mobiletablestyle === 'default'
						"
					>
						<template
							v-for="(data, j) in storageData"
							...
							>
								<td
									...
									v-bind:class="{
										...
										'p-1': model.densetable === 'true',
										'align-top':
											model.rowalignment === 'top' ||
											model.rowalignment === '',
										'align-center':
											model.rowalignment === 'center',
										'align-bottom':
											model.rowalignment === 'bottom',
									}"
								>
									<span
										class="action relative cursor-pointer"
					...
					<tbody class="mobile-scroll" v-else>
						<tr class="action-row">
							<td
								class="action-item-all mobile-header"
								v-bind:class="{
									border: model.cellborders === 'true',
									'p-3': model.densetable !== 'true',
									'p-1': model.densetable === 'true',
...
<script>
	export default {
		...
		data() {
			...
			if (
				(!this.model.endpointurl || this.model.endpointurl === "") &&
				...
				try {
					...
					console.error(
						"JSON parsing error loading storage: " + storage
					);
				}
		...
		computed: {
			isEditAndEmpty() {
				...
				return this.$helper.areAllEmpty(
					this.model.columns && this.model.columns.length > 0
				);
			},
		},
		mounted() {
		...
		methods: {
			...
			loadAction: function () {
				if (this.model.loadfunction && this.model.loadfunction !== "") {
					...
					while (obj && parent[obj]) {
						if (objs.length === 0) {
							try {
								const result = parent[obj](
									this.model.endpointurl
								);
								if (result === false) {
									console.error(
					...
					console.log(
						"window." + this.model.loadfunction + " not found"
					);
					return;
				} else if (
					this.model.endpointurl &&
			...
			deleteAction: function () {
				if (
					...
					console.log(
						"window." + this.model.deletefunction + " not found"
					);
					return;
				} else if (
					this.model.loadfunction &&
					this.model.loadfunction !== ""
				) {
					console.error(
						"Data loaded externally, we cannot delete without a configured delete function"
					);
				} else {
