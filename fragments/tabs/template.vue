    ...
    <div class="w-full" v-else>
      <div>
        ...
        <h2
          ...
          data-per-inline="model.subtitle"
        >
          {{ model.subtitle }}
        </h2>
      </div>
      ...
      >
        ...
        <div class="py-3 flex flex-col mx-3 lg:flex-grow">
          ...
          <div class="flex flex-wrap justify-center pb-3" role="tablist">
            <a
              class="mx-2 no-underline p-3 cursor-pointer"
              role="tab"
            ...
            'text-dark' : active !== i &amp;&amp; model.tabcolor === 'dark' || active === i &amp;&amp; model.tabcolor === 'light',
              ...
              v-bind:aria-selected="active === i"
              v-on:click="toggleActive(i)"
          ...
          <div class="relative">
            <div
              class="w-full py-3 opacity-0 transition-opacity"
              role="tabpanel"
              v-for="(item, i) in model.tabs"
              :key="item.path || i"
              v-bind:id="`tab${_uid}${parseInt(i) + 1}`"
              v-bind:aria-labelledby="`tab-control-${_uid}${parseInt(i) + 1}`"
              v-bind:class="active === i ? 'block opacity-100' : 'hidden'"
              v-html="item.text"
              v-bind:data-per-inline="`item.text`"
            ></div>
          </div>
        </div>
      </div>
